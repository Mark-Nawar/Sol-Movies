...
//Connect to mongoDB
//mongodb+srv://solanastar:starsolana@cluster0.llghh.mongodb.net/?retryWrites=true&w=majority
const dbURI = 'mongodb+srv://solanastar:starsolana@cluster0.llghh.mongodb.net/?retryWrites=true&w=majority';
...
app.get('/getdetailsofEvent',async(req,res)=>{
    let eventID = req.body.id;

    let found = await MovieEvent.findById(eventID);
    let found2 = await Movie.findById(found.M_id);
    if(found){
        res.json({name:found2.name, found:found});
    }
...
app.get("/viewReser", async (req,res) =>
{
    ...
    if(jtoken != null)
    {
        try
        {
            ...
            for(let i in resrv){
                console.log(i);
                let found = await MovieEvent.findById(resrv[i].movieEvent);
                console.log(found);
                let found2 = await Movie.findById(found.M_id);
                if(found){
                    arr.push({name:found2.name, found:found});
                }
            }
            console.log(arr);
            res.json(arr);
